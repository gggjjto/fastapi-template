...
| `--with-worker` | Included by default | `apps/api/app/worker.py`, `apps/api/app/core/arq.py`, `apps/api/tests/core/test_tasks.py` | `APP_REDIS_URL` | Requires `--with-redis` because Arq uses Redis as broker. |
...
python3 scripts/create_project.py bad-worker ../bad-worker --with-worker
```
